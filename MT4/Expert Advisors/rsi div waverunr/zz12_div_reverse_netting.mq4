...
//netting is optional.
//this is a copy of the mt4 version of this ea called "zz12_div_netting". the only differences is that buy trades are taken in place of a sell condition.
//this ea is the reverse logic of the ea zz12 div netting.  buy is at the break of zz12 pattern down after a sell div. tp at the <div or zz> high and sl <1:1 > of the tp. 
//sell is at the break of zz12 up after a buy div. tp at the <div or zz low> and sl <1:1 > of the tp. 

//minimum mtgl range added in the inputs section and in ea logic. if the range of the netting is less than specified min range (in the inputs section)
//then it won't take a trade

//for the netting, currently, you have to choose what the range will be based on. either divergence based or 1st zz swing based or 2nd zz swing based.
//you can also choose if u want it to be based on whatever the sl is.

//this strategy is a trend continuation strategy - the zz12 is considered as the correction after which the trend is supposed to resume.
...
//this ea contains ORCHARD MTGL (netting). there is one of his martingales in this ea. this mtgl only opens one entry at a time and opens it at the zone high/low 
...
//removed risk based lotsize because it's causing an error to happen and stopping the backtest. Also, it is not being used for testing
...
input double pips_for_tp = 0; //Amount of pips to be added/subtracted from tp
...
input string blb = " "; //_
input bool zigzag_tp = false; //Allow ZigZag based takeprofit
input bool first_hi_low = false; //Allow TP to be 1st ZZ high/low Based
input string a1 = " "; //_
...
input string b2 = " "; //_
input bool div_tp = true; //Allow Divergence based takeprofit
input string c3 = " "; //_
input bool sl_rsk_rwrd = false; //Allow Risk-Reward Based Stoploss
input double risk_reward_sl = 2; //Risk to Reward Ratio for first trade's SL
input string f6 = " "; //_
...
input double mtgl_min_range = 10; //Minimum mtgl range in Pips
...
bool use_inital_lots = true; //Use Initial Lots
input double inital_lots = 0.01; //Initial Lots
double Risk = 0.01; // % Risk as per equity (0.01 = 1%)
...
regular_divergence(zz_arrow_depth3, zz_arrow_deviation3, zz_arrow_backStep3, lower_timeframe, div_lookback,
  ...
  {  
   ...
   //Print("buy | cond6 = ",(zigzagdowntime <= zigzaguptime && zigzagdown < zigzagdown2 && zigzagup < zigzagup2 && zigzagdown2>0 && zigzagup2>0 && iHigh(_Symbol, lower_timeframe, 0) >= zigzagup2),"");
   //Print("buy | ",zigzagdowntime >= zigzaguptime && iLow(_Symbol, lower_timeframe, 0) < zigzagup && zigzagdown < zigzagdown2 && zigzagdown2>0 && zigzagup2>0 && iHigh(_Symbol, lower_timeframe, 0) >= zigzagup,"");
   ...
   {                        
         ...
         double tpt = div_tp ? high_of_reg_bear2 : zigzag_tp ? zigzagdown : tp_fixd ? price + (_Point*(fixed_pips_tp*10)) : first_hi_low ? zigzagdown2 : first_trade_exit ? zigzagdown:0;
         double tp = tpt != 0 ? tpt + (_Point*(pips_for_tp*10)):0;
         double sl = sl_rsk_rwrd ? NormalizeDouble(price - ( price > tp ? ((price - tp) * risk_reward_sl) : ((tp - price) * risk_reward_sl) ), _Digits) : sl_fixd ? price - (_Point*(fixed_pips_sl*10)) : first_trade_exit ? price - (tp-price):0;
         
         //double point = SymbolInfoDouble(_Symbol,SYMBOL_POINT); 
         //double pips = (price - sl)/_Point;
         ...
         if(date.hour >= _start && date.hour < end && pip_range(price, sl))
         ticket = OrderSend(_Symbol, OP_BUY, inital_lots, price, 100, sl, tp, "buy", 1);
   ...
   //Print("cond1 = ",trade_close_shift <= reg_bull_div_curr_time,"  cond2 = ",reg_bull_div_curr_time != 0,"  cond3 = ",reg_bull_div_curr_time != prev_reg_bull_div_curr_time,"  cond4 = ",lowest1 == low_of_reg_bull2,"  cond5 = ",zigzaguptime2 >= reg_bull_div_curr_time,"");
    //Print("sell cond6 = ",((zigzagdowntime >= zigzaguptime && zigzagdown > zigzagdown2 && zigzagup > zigzagup2 && zigzagdown2>0 && zigzagup2>0 && iLow(_Symbol, lower_timeframe, 0) <= zigzagdown2) || 
       ...
       (zigzagdowntime <= zigzaguptime && iHigh(_Symbol, lower_timeframe, 0) > zigzagdown && zigzagup > zigzagup2 && zigzagdown2>0 && zigzagup2>0 && iLow(_Symbol, lower_timeframe, 0) <= zigzagdown)) )
   {
         ...
         double tpt = div_tp ? low_of_reg_bull2 : zigzag_tp ? zigzagup : tp_fixd ? price - (_Point*(fixed_pips_tp*10)) : first_hi_low ? zigzagup2 : first_trade_exit ? zigzagup:0; 
         double tp = tpt != 0 ? tpt - (_Point*(pips_for_tp*10)):0;
         double sl = sl_rsk_rwrd ? NormalizeDouble(price + ( price > tp ? ((price - tp) * risk_reward_sl) : ((tp - price) * risk_reward_sl) ), _Digits) : sl_fixd ? price + (_Point*(fixed_pips_sl*10)) : first_trade_exit ? price + (price - tp) : 0;  
         
         //double point = SymbolInfoDouble(_Symbol,SYMBOL_POINT); 
         //double pips = (sl - price)/_Point;
         //double freeMargin = AccountInfoDouble(ACCOUNT_MARGIN_FREE);
         //double PipValue = (((SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_VALUE))*point)/(SymbolInfoDouble(_Symbol,SYMBOL_TRADE_TICK_SIZE)));
         //double Lots = Risk * freeMargin / (PipValue * pips);
         //Lots = floor(Lots * 100) / 100;
         ...
         //Print("tp = ",tp,"  sl = ",sl,"");
         if(date.hour >= _start && date.hour < end && pip_range(price, sl))
         ticket = OrderSend(_Symbol, OP_SELL, inital_lots, price, 100, sl, tp, "sell", 1);
...
{
  bool check= false;
  if((val1 > val2 ? val1 - val2:val2 - val1) >= mtgl_min_range*10*_Point)
  check = true;
