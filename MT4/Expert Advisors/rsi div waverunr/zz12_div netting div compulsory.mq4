...
//netting is optional.
//this is a copy of "zz12_div netting". 
//this ea takes sell/buy entry if all 3 divergences come at the same point. (if their 2nd high/low comes at the same point as the other 2 divergences)
//the sl will be based on the divergence which has the highest degree. (if the user wants a divergence based sl)
//the entry is going to be based on the highest divergence degree
...
//the sl and entry conditions depend on the highest degree of divergence. 
//so, find the highest degree in the selected divergences and input it's depth, deviation, backstep
//compare the selected divergence's Depth, and find out which has the maximum value. (use MathMax)

//and for the divergence condition which determines the buy/sell entry, check if all individual
//divergences' 2nd high/low's time matches with the other 2

//this isn't completed or updated
...
{
divergence(zz_arrow_depth, zz_arrow_deviation, zz_arrow_backStep, lower_timeframe, 
            high_of_reg_bear2, low_of_reg_bull2, reg_bear_div_time, reg_bull_div_time, reg_bear_div_curr_time, reg_bull_div_curr_time, reg_bull_lp, reg_bear_lp);
}

if (use_zz2 == true)  //for regular divergence
{
divergence(zz_arrow_depth2, zz_arrow_deviation2, zz_arrow_backStep2, lower_timeframe, 
           high_of_reg_bear2, low_of_reg_bull2, reg_bear_div_time, reg_bull_div_time, reg_bear_div_curr_time, reg_bull_div_curr_time, reg_bull_lp, reg_bear_lp);
}
          
if (use_zz3 == true) //for regular divergence
{
divergence(zz_arrow_depth3, zz_arrow_deviation3, zz_arrow_backStep3, lower_timeframe, 
           high_of_reg_bear2, low_of_reg_bull2, reg_bear_div_time, reg_bull_div_time, reg_bear_div_curr_time, reg_bull_div_curr_time, reg_bull_lp, reg_bear_lp);
}
