...
//p[ickup from where it left off and the current mtgl won't continue.
...
void OnTick()
{
  ...
  //Print("ma = ",std_ma_buffer[0],"  std = ",std_buffer[0],"");
  ...
  for(int i= zz4_shift ; i<lookback_range; i++)
   ...
   barshift = iBarShift(_Symbol, timeframe, zigzagdowntime);
   barshift = barshift <= 0 ? 1:barshift; 
  ...
  if(bullish_cross_shift > 0)
  {
  for(int i = shift; i >= 1; i--)
  {
    if((Bars-1) <= ((bullish_cross_shift + candles)-1)) 
    {
      if((std_buffer[i-1] > std_ma_buffer[i-1] /*|| std_buffer[i-1] < std_ma_buffer[i-1]*/) && i-1 == 0 && trade_close_shift < bullish_cross_shift && bullish_cross_shift == latest_cross)
      {
